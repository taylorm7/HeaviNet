function [y_digital, y] = create_level_decimate_mu(level, song, fx, total_levels)
    ...
    fprintf('Decimate/Mu Level:%d bits:%d x_fx:%g N:%d mu:%d Q:%g \n', level-1, level,x_fx, N,mu, Q);
    ...
    y_nonlinear_down = mu_trasform(x_down, mu, Q);
    y_digital_down = analog_to_digital(y_nonlinear_down, Q);
    % compute analog to digital, and perform inverse mu-law transform
    y_analog_down = digital_to_analog(y_digital_down, Q);
    y_down = mu_inverse(y_analog_down, mu, Q);
    
    x_up = up_sample(y_down, fx_factor);
    
    % compute error for downsampling and upsampling
    D=x_up-song;
    MSE=mean(D.^2);
    fprintf('Error between original and downsampled/mu law = %g\n',MSE )
