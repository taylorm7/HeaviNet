...
def write_data(data_dict, fx, data_location, level, receptive_field, epochs, name_string):
    seed_name = os.path.split(data_location)[1].split('.')[0]
    training_name = (os.path.split( os.path.split(data_location)[0])[1]).split('.')[0]

    song_name = name_string+str(level)+"_r"+str(receptive_field)+"_"+str(epochs)+"_"+training_name+"_"+seed_name 
    song_file = data_location + "/" + song_name + ".mat"
